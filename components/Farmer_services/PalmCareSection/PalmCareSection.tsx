...
import styles from "./PalmImage.module.css";
...
const PalmCareSection: React.FC<PalmCareSectionProps> = ({
  ...
  return (
    <section className="bg-[#f3e9df] py-12 px-6 md:px-10  mx-auto rounded-lg flex min-h-[70vh] flex-col md:flex-row items-center gap-8">
      {/* النص على اليسار */}
...
>
  <span className="relative z-10">اطلب مستلزماتك</span>
  <span
...
<button
  ...
  className="relative cursor-pointer overflow-hidden
    ...
    group active:scale-95 transition-transform fade-in-down delay-500"
>
  <span className="relative z-10 hover:text-white">واتساب</span>
  <span
    className="absolute inset-0 left-0 bottom-0 w-full h-full bg-[#3b2617]
      transform -translate-x-full -translate-y-full -skew-x-1
      opacity-0 transition-all duration-300 ease-out
  ...
  ></span>
      ...
      {/* الصورة على اليمين */}
   <div className={` w-full md:w-[45%] max-w-[660px] max-h-[460px]  rounded-3xl overflow-hidden shadow-lg`}>
  <Image
    src={imageSrc}
    alt={title}
    width={760}
