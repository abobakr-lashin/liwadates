...
export default function HeroCarousel({
  controlsClassName = "bottom-3  -translate-x-1/2 sm:bottom-5 sm:right-5 sm:translate-x-0",
}: Props) {
  ...
  return (
    <div className="relative w-full max-w-[100vw] overflow-hidden rounded-3xl">
      <Swiper
        ...
        onBeforeInit={(swiper) => {
          // @ts-expect-error
          swiper.params.navigation = { ...(swiper.params.navigation as object), prevEl: prevRef.current, nextEl: nextRef.current };
          // @ts-expect-error
          swiper.params.pagination = { ...(swiper.params.pagination as object), el: pagRef.current, clickable: true };
        }}
        onSwiper={(swiper) => {
          setTimeout(() => {
            if (prevRef.current && nextRef.current) {
              ...
              swiper.navigation.destroy(); swiper.navigation.init(); swiper.navigation.update();
            }
            if (pagRef.current) {
              // @ts-expect-error
              swiper.params.pagination.el = pagRef.current;
              swiper.pagination.destroy(); swiper.pagination.init(); swiper.pagination.update();
            }
          });
        }}
      >
      ...
      {/* شريط التحكم */}
      <div
        className={"absolute z-20 flex items-center gap-2 rounded-lg border w-auto border-white/30 bg-black/35 backdrop-blur-sm px-2 py-1 " +
          controlsClassName
        }
        style={{ maxWidth: "calc(100% - 1rem)" }}
      >
        <button ref={prevRef} className="grid place-items-center w-9 h-9 rounded-md border border-white/40 text-white hover:bg-white/10" aria-label="السابق">
          <HiOutlineChevronRight size={18} />
        </button>

        <div ref={pagRef} className="hero-pagination swiper-pagination  !w-auto !relative px-1" />

        <button ref={nextRef} className="grid place-items-center w-9 h-9 rounded-md border border-white/40 text-white hover:bg-white/10" aria-label="التالي">
